...
function SvgCrop({ label, viewBox, className, preserveAspectRatio = "xMinYMin meet" }: SvgCropProps) {
  return (
    <svg
...
function Sidebar() {
  return (
    <aside className="hidden min-h-svh w-[188px] shrink-0 flex-col justify-between bg-[#FDEFFD] px-4 py-6 lg:flex">
      <div>
        <SvgCrop className="h-[34px] w-[140px]" label="SaTHI" viewBox="16 16 140 34" />
      ...
      <a className="flex h-9 w-[156px] items-center gap-3 px-2 text-sm leading-[1.4] text-[#616161]" href="#">
  ...
  return (
    <header className="hidden h-16 lg:flex lg:justify-center">
      <div className="flex h-full w-full max-w-[1228px] items-center justify-between">
        <h1 className="font-sans text-lg font-semibold leading-[1.4] text-[#303030]">
          Gamification
        </h1>
        <div className="flex items-center gap-4">
          <SvgCrop className="h-8 w-[34px]" label="Notifications" viewBox="1218 16 30 32" />
          <SvgCrop className="size-8 rounded-full" label="User avatar" viewBox="1262 16 32 32" />
        </div>
      </div>
    </header>
  )
  ...
  return (
    <section className="flex w-full max-w-[354px] flex-col items-center gap-6 text-center">
      <div className="flex flex-col items-center gap-2">
        <h2 className="font-sans text-[28px] font-semibold leading-[1.4] text-[#561056]">
      ...
      <button className="h-10 w-[310px] rounded-[10px] bg-[#C530C5] text-base leading-[1.4] text-white" type="button">
        Enable Gamification
      </button>
    </section>
  )
    ...
    <div className="grid w-full grid-cols-1 justify-center gap-6 md:grid-cols-[repeat(3,292px)] lg:grid-cols-[repeat(3,var(--card-w))] lg:gap-[var(--card-gap)]">
      {cards.map((card) => (
        <article
          ...
          key={card.title}
        >
          <CardWave className="pointer-events-none absolute -left-[30px] -top-[58px] h-[211px] w-[545px] lg:left-[var(--wave-left)] lg:top-[var(--wave-top)] lg:h-[var(--wave-h)] lg:w-[var(--wave-w)]" />
          <SvgCrop className="absolute left-1/2 top-[22px] size-[70px] -translate-x-1/2 lg:top-[var(--icon-top)] lg:size-[var(--icon-size)]" viewBox={card.iconCrop} />
          <div className="absolute inset-x-4 top-[111px] flex flex-col items-center gap-2 text-center lg:top-[var(--copy-top)] lg:gap-[var(--copy-gap)]">
            <h3 className="font-sans text-base font-medium leading-[1.4] text-[#303030] lg:text-[length:var(--title-size)]">
              {card.title}
            </h3>
            <p className="max-w-[270px] text-sm leading-[1.4] text-[#616161] lg:max-w-[var(--body-w)] lg:text-[length:var(--body-size)]">{card.body}</p>
          </div>
        </article>
      ))}
    </div>
    ...
    <div className="min-h-svh bg-white text-[#303030]">
      ...
      <div className="min-h-svh lg:flex">
        ...
        <div className="min-w-0 flex-1">
          <TopNav />
          <main className="px-4 pb-12 pt-8 sm:px-6 lg:px-0 lg:pt-[38px]">
